...
public class mainC extends javax.swing.JFrame {
    ...
    private void minim_btnMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_minim_btnMousePressed
        // TODO add your handling code here:
        resul_ta.append("SIMPLEX MINIMIZACION\n");
        DecimalFormat decimalFormat = new DecimalFormat("0.00");
        int fila = 1 + Integer.parseInt(rest_txt.getText());
        int columna = Integer.parseInt(var_txt.getText());
        double[][] v = new double[fila][fila + columna];        
        ...
        for (int i = 0; i < columna; i++) {
            v[0][i] = -Double.parseDouble(jTable1.getValueAt(0, i).toString());
        }
        ...
        while (co < columna) {
            ...
            resul_ta.append("EL MÃXIMO POSITIVO ES: " + positivo + "\nla columna es: ");
            for (int i = 0; i < fila; i++) {
                resul_ta.append(v[i][c] + "\t");
            }
            resul_ta.append("\n\n dividiendo con la columna\n");
            ...
            resul_ta.append("el menor de la divisiÃ³n es: " + menor + "\n");
